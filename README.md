...
- **Частичное выполнение** — этапы с датами начала/окончания; кнопки **Выполнено**, **Сдвиг** и **Удалить**; шаблоны этапов из `Template_substages` и свои шаблоны проекта; автоподстановка дат между соседними этапами
- **Зависимости этапов** — внутри задачи (номера этапов или визуальный редактор) и между задачами с привязкой к конкретным этапам (`Задача А:2>3` в столбце предшественников)
...
| `GET/POST /api/tasks/{id}/sub-stages` | Список или создание этапов (`start_date`, `end_date`, `is_indicative`, `predecessor_stage_ids`; пересчёт сроков задачи/компонента) |
| `PATCH /api/tasks/{id}/sub-stages/{sub_id}` | Обновление этапа (даты, `is_done`, внутренние предшественники; синхронизация с общим компонентом) |
